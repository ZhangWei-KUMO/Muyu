...
const MovieDetails = ({navigation, route}) => {
  ...
  const handlePerson = share_url => {
    setCreditId(share_url);
    handlePersonModal();
  };
  ...
  const requestMoviesInfo = async () => {
    try {
      ...
      setInfo({
        share_url,
        backdropPath: INITIAL_INFO.backdropPath,
        title: INITIAL_INFO.title,
        voteAverage: INITIAL_INFO.voteAverage,
        video: INITIAL_INFO.video,
        overview: INITIAL_INFO.overview,
        cast: sliceArrayLength(0, 15),
        crew: sliceArrayLength(0, 15),
        productionCompanies: sliceArrayLength(0, 10),
    ...
    navigation.setOptions({
      headerRight: () => (
        <TouchableOpacity
          ...
          onPress={() =>
            handleShare(route.params.title, route.params.share_url)
          }>
